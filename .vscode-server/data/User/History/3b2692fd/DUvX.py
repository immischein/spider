...
class Spider:
    ...
    def walk_forward_diagonal(self, steps, speed=0.5):
        for _ in range(steps):
            ...
            # Reset legs to starting position
            self.start_position()
    def tripod_turn(self, steps, direction, speed = 0.5, change_deg):
        if direction == 'right':
            angle = -30
            go = True
        elif direction == 'left':
        ...
        if go:
            ...
            # Lift and move legs 0,1, 2 3, 4, 5 forward
                for leg_index in [[0, 3, 4], [2, 1, 5]]:
                    ...
                    sleep(.1)
                    self.move_single_leg([current_deg_1[0] + change_deg, 150, current_deg_1[2]], leg_index[0])
                    self.move_single_leg([current_deg_2[0] + change_deg, 150, current_deg_2[2]], leg_index[1])
                    self.move_single_leg([current_deg_3[0] + change_deg, 150, current_deg_3[2]], leg_index[2])

                    sleep(.1)
                    self.move_single_leg([current_deg_1[0] + change_deg, deg2, deg3], leg_index[0])
                    self.move_single_leg([current_deg_2[0] + change_deg, deg2, deg3], leg_index[1])
                    self.move_single_leg([current_deg_3[0] + change_deg, deg2, deg3], leg_index[2])
